...
public extension ViewSetterProtocol where View: StackViewExtended {
   ...
   @discardableResult func backImage(
      _ value: UIImage,
   ...
   @discardableResult func frontViewModel(
      _ value: UIViewModel,
      anchors: ((Anchors, UIView) -> Void) = { $0.fitToView($1) }
   ) -> Self {
      let subview = value.uiView
      view.addSubview(subview)
      view.frontView = subview
      anchors(subview.addAnchors, view)
      return self
   }

   @discardableResult func disableBottomRadius(
      _ value: CGFloat,
      matteColor: UIColor? = nil,
      isOnTop: Bool = false
   ) -> Self {
      let backView = UIView()
